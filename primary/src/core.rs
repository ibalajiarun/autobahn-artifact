...
use crate::messages::{
    transform_commitQC, verify_commit, verify_confirm, Certificate, CommitQC, ConsensusMessage,
    ConsensusRequest, ConsensusType, ConsensusVote, Header, Proposal, Timeout, Vote, QC, TC,
};
        ...
        match &commit_message {
            ConsensusMessage::Commit {
                ...
                if self.simulate_asynchrony && *slot == 1 {
                    ...
                    let async_end =
                        Timer::new(0, 0, self.asynchrony_start + self.asynchrony_duration);
